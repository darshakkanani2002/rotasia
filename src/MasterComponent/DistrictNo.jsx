...
export default function DistrictNo() {
    ...
    const fetchData = () => {
        axios.get('http://localhost:3030/DistrictNo').then(response => {
            console.log("District No data===>", response.data);
    ...
    const handleSubmit = (event) => {
        ...
        axios.post('http://localhost:3030/DistrictNo', districtvalue).then(response => {
            console.log("Disrict Data Add Successfully", response.data);
            fetchData();
            setDistrictValue({ District_No: '' });
        }).catch(error => {
            console.log(error);
        })
    }
    ...
    const handleDelete = (id) => {
        axios.delete(`http://localhost:3030/DistrictNo/${id}`).then(response => {
            console.log("Deleted Successfully district data ===>", response.data);
            fetchData();
        }).catch(error => {
            console.log(error)
        })
                ...
                <div>
                    <div className='d-flex flex-wrap justify-content-center justify-content-lg-between align-items-center'>
                        <div>
                            ...
                            {/* <!--import-Modal --> */}
                            <div className="modal fade" id="exampleModal19" tabIndex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div>
                            <button type='button' className='create-admin-btn fw-medium px-4 py-2' data-bs-toggle="modal" data-bs-target="#exampleModal7">Create District No.</button>
                            {/* <!-- Modal --> */}
                                    ...
                                    <div className="modal-content pt-4">
                                        <form onSubmit={handleSubmit}>
                                            ...
                                            <div className="modal-body my-4">
                                                <label htmlFor="" className='master-popup-lable mb-2'>District No.</label>
                                                <input type="text" value={districtvalue.District_No} placeholder='Enter here' className='master-popup-input px-3' onChange={e => setDistrictValue({ ...districtvalue, District_No: e.target.value })} />
                                            </div>
                    ...
                    <div className='table-responsive'>
                        <table className='table'>
                            ...
                            <tbody>
                                ...
                                {/* <tr>
                                    <td>3060</td>
                                    <td><img src='/img/ic_delate.svg' className='img-fluid mx-1' alt='ic_delate' onClick={() => handleDelete(index)}></img>
                                        <img src='/img/ic_edit_01.svg' className='img-fluid mx-1' alt="ic_edit_01" /></td>
                                </tr> */}
                            </tbody>
                            {/* <tbody>
                                {districtNumber.length === 0 ? (
                                    <tr className='text-center'>
                                        <td colSpan="2">No records found</td>
                                    </tr>
                                ) : (
                                    districtNumber.map((disrict, index) => (
                                        <tr key={index}>
                                            <td>{disrict}</td>
                                            <td>
                                                <img src='/img/ic_delate.svg' onClick={() => districtDelete(index)} className='img-fluid mx-1' alt='ic_delate' ></img>
                                                <img src='/img/ic_edit_01.svg' className='img-fluid mx-1' alt="ic_edit_01" />
                                            </td>
                                        </tr>
                                    ))
